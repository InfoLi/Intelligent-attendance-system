...
static int detect_retinaface(const cv::Mat& bgr, std::vector<FaceObject>& faceobjects)
{
    ...
    retinaface.load_param("./model/mnet.25-opt.param");
    retinaface.load_model("./model/mnet.25-opt.bin");

//    retinaface.load_param("./model/retinaface.param");
//    retinaface.load_model("./model/retinaface.bin");
...
static cv::Mat draw_faceobjects(const cv::Mat& bgr, const std::vector<FaceObject>& faceobjects)
{
...
cv::Mat getTrainResult(){
    const char* imagepath = "./peoples.jpg";
    ...
    return draw_faceobjects(m, faceobjects);
}

cv::Mat getResult(cv::Mat input){
    std::vector<FaceObject> faceobjects;
    detect_retinaface(input, faceobjects);
    cv::Mat re = draw_faceobjects(input, faceobjects);
    cv::flip(re,re,1);//左右镜像变换
