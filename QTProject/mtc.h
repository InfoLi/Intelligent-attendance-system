...
static cv::Mat faceDetect(const cv::Mat& bgr, std::vector<float>& result)
{
    MTCNN mtcnn;
    mtcnn.SetMinFace(40);
...
//        }
    ...
    cv::Mat imgSave = image(bbox[0]);
    imwrite("imsave.jpg",imgSave);

    return image;
}

cv::Mat faceDetectionAPI (cv::Mat m)
{
...
//    detect_retinaface(m, faceobjects);
    ...
    cv::Mat re = faceDetect(m, cls_scores);
    return re;
}
